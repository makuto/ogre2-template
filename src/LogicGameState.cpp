...
namespace Mogre
{
LogicGameState::LogicGameState() : mDisplacement(0), mCubeEntity(0), mCubeMoDef(0), mLogicSystem(0)
{
}
//-----------------------------------------------------------------------------------
...
void LogicGameState::createScene01(void)
{
	...
	Demo::GameEntityManager* geMgr = mLogicSystem->getGameEntityManager();

	mCubeMoDef = new Demo::MovableObjectDefinition();
	mCubeMoDef->meshName = "Cube_d.mesh";
	mCubeMoDef->resourceGroup = Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME;
	mCubeMoDef->moType = Demo::MoTypeItem;
